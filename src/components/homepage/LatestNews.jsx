...
const LatestNews = () => {
  ...
  return (
    <>
      <div className="bg-latestNews_bg_img bg-no-repeat bg-cover pt-[120px] pb-[205px]">
        <div className="container">
          <p className="text-center text-white font-bold text-4xl pb-[51px]">
            Latest news
          </p>
          <div className="flex flex-col xl:flex-row items-center backdrop-blur-[6px] rounded-[12px] bg-[rgba(255, 255, 255, 0.08)] xl:h-[468px]">
            <div className="max-w-[350px] sm:max-w-[485px]">
              <Slider {...settings}>
                <img
            ...
            <div className="mx-auto w-full lg:w-3/4 xl:w-full md:px-4 mt-10 xl:mt-0 h-[527px] xl:h-full">
              <div className="h-[437px]">
                {Latest_news_faq_data.map((item, index) => (
                  <div
                    key={index}
                    onClick={() => activeHandler(index)}
                    className={`faq-wrapper cursor-pointer text-white border-b border-gray-500 py-2 sm:py-3 lg:py-4 ${
                      (index === view ? "active" : "", item.className)
                    }`}
                  >
                    <div
